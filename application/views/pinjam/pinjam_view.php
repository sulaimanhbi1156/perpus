									...
									<?php 
										if($isi['status'] == 'Di Kembalikan')
										{
											...
											if($diff > 0 )
											{
												...
												$biayaDenda = 0;
												$biayaDenda = $jml*($dd->harga_denda*$diff);
									...
									<?php if($this->session->userdata('level') == 'Petugas'){ ?>
										<?php if($isi['tgl_kembali'] == '0') {?>
											<a href="<?= base_url('transaksi/kembalipinjam/'.$isi['pinjam_id']);?>" class="btn btn-warning btn-sm" title="pengembalian buku">
												...
												<?php 
												if(isset($biayaDenda)) { ?>
											<a href="<?= base_url('transaksi/wapinjam/'.$isi['pinjam_id']);?>" 
											class="btn btn-success btn-sm" title="detail pinjam">
											<i class="fa fa-whatsapp"></i> Kirim WA</a>
										<?php } else { ?>
											<a href="<?= base_url('transaksi/wapinjam2/'.$isi['pinjam_id']);?>" 
											class="btn btn-success btn-sm" title="detail pinjam">
											<i class="fa fa-whatsapp"></i> Kirim WA</a>
										<?php } ?>
										<?php }else{ ?>
