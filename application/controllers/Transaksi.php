...
class Transaksi extends CI_Controller {
	...
	{	
		$this->data['title_web'] = 'Data Histori Peminjaman ';
		$this->data['idbo'] = $this->session->userdata('ses_id');
	...
	public function kembalipinjam()
	{
		...
		$image_name='qrkembali-'.$id.'.png'; //buat name dari qr code sesuai dengan nim

		$params['data'] = base_url('transaksi/prosespinjam?kembali='.$id); //data yang akan di jadikan QR CODE
	...
	public function notadenda($id)
	{
		...
		WHERE status = 'Di Kembalikan' AND denda > 0 AND tbl_denda.pinjam_id = '$id'");		
        $this->data['title_web'] = 'NOTA PEMBAYARAN DENDA';

        $this->load->view('denda/nota',$this->data);
	}

	public function edit_status_denda() {
			$post= $this->input->post();
			$data = array(
				'status_denda'=>($post['status_denda'])
			);

			$this->db->where('pinjam_id',$post['pinjam_id']);
			$this->db->update('tbl_denda', $data);


			$this->session->set_flashdata('pesan','<div id="notifikasi"><div class="alert alert-success">
			<p> Edit Status Denda  Sukses !</p>
			</div></div>');
			redirect(base_url('transaksi/tabel_denda')); 
	}
	...
	public function wapinjam($id) {
		$pinjam = $this->db->query("SELECT * FROM tbl_pinjam INNER JOIN tbl_login on tbl_login.anggota_id = tbl_pinjam.anggota_id
		WHERE pinjam_id = '$id'");
		...
		foreach($pinjam->result_array() as $isi){
				...
				'target' => $target,
				'message' => 'Assalamualaikum '.$nama.', dimohon untuk mengembalikan buku yang dipinjam karena sudah telat '.$lama_waktu.' Hari, dengan denda Rp '.$harga_denda.', Terimakasih'),
				...
				echo $response;
				$this->session->set_flashdata('pesan','<div id="notifikasi"><div class="alert alert-success">
				<p> Kirim WA BERHASIL !</p>
				</div></div>');
				redirect(base_url('transaksi')); 

	}

	public function wapinjam2($id) {
		$pinjam = $this->db->query("SELECT * FROM tbl_pinjam RIGHT JOIN tbl_login on tbl_login.anggota_id = tbl_pinjam.anggota_id
		WHERE pinjam_id = '$id'");
		...
		foreach($pinjam->result_array() as $isi){
				...
				CURLOPT_POSTFIELDS => array(
				'target' => $target,
				'message' => 'Assalamualaikum '.$nama.', dimohon untuk mengembalikan buku yang ada tepat waktu sebelum tanggal '.$tglBalik.' sehingga tidak perlu membayar denda , Terimakasih'),
				CURLOPT_HTTPHEADER => array(
					"Authorization: $token"
				),
				));
	...
	public function tabel_denda()
	{
		...
		$this->data['title_web'] = ' Data Denda ';
		$this->load->view('header_view',$this->data);
		$this->load->view('sidebar_view',$this->data);
		$this->load->view('denda/tabel_denda',$this->data);
		$this->load->view('footer_view',$this->data);
	}
}
